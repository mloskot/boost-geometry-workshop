...
#ifndef BOOST_GEOMETRY_IO_WKB2_DETAIL_PARSER_HPP
#define BOOST_GEOMETRY_IO_WKB2_DETAIL_PARSER_HPP
...
struct value_parser
{
    ...
    static bool parse(Iterator& it, Iterator end, T& value, BOOST_SCOPED_ENUM(byte_order) order)
    {
        ...
        if (it != end && std::distance(it, end) >= required_size)
        {
            ...
            if (byte_order::xdr == order)
            {
                parsed_value.template load<endian::big_endian_tag>(it);
            }
            else if (byte_order::ndr == order)
            {
...
{
    template <typename Iterator>
    static bool parse(Iterator& it, Iterator end, BOOST_SCOPED_ENUM(byte_order)& order)
    {
        boost::uint8_t value;
...
{
    template <typename Iterator>
    static void run(Iterator& it, Iterator end, P& point, BOOST_SCOPED_ENUM(byte_order) order)
    {
...
{
    template <typename Iterator>
    static bool parse(Iterator& it, Iterator end, C& container, BOOST_SCOPED_ENUM(byte_order) order)
    {
...
{
    template <typename Iterator>
    static bool parse(Iterator& it, Iterator end, L& linestring, BOOST_SCOPED_ENUM(byte_order) order)
    {
...
#endif // BOOST_GEOMETRY_IO_WKB2_DETAIL_PARSER_HPP
